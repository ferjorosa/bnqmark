"""Database utilities for the code generation strategy analysis table."""
...
# Constants
TABLE_NAME = "discrete_code_generation_strategy_analysis"
...
def initialize_code_generation_strategy_analysis_db() -> None:
    """
    Initialise the SQLite table for code generation strategy analysis.
    ...
    if result_df.empty:
        create_query = f"""
            CREATE TABLE {TABLE_NAME} (
                ...
                strategy_classification TEXT NOT NULL,
                thinking_style_classification TEXT NOT NULL,

                -- Simple fields
                imported_libraries TEXT NOT NULL,
...
def get_existing_code_generation_strategy_analysis_identifiers() -> set[
...
def insert_code_generation_strategy_analysis(
    query_uuid: str,
    naming_strategy: str,
    run: int,
    model_name: str,
    experiment_type: str,
    analysis_object: CodeStrategyAnalysis,
    ...
    """Insert a row into the discrete_code_generation_strategy_analysis table."""
    # Extract fields from Pydantic model
    strategy_classification = analysis_object.strategy_classification.model_dump_json()
    thinking_style_classification = (
        analysis_object.thinking_style_classification.model_dump_json()
    )

    # Simple JSON serialization for list of strings
    imported_libraries = json.dumps(analysis_object.imported_libraries)

    code_summary = analysis_object.code_summary
    ...
    query = f"""
        INSERT INTO {TABLE_NAME} (
            ...
            strategy_classification, thinking_style_classification,
            imported_libraries, code_summary,
    ...
    logger.info(
        f"Inserted code generation strategy analysis for query {query_uuid}, "
