"""Database utilities for the arithmetic behavior analysis table."""
...
# Constants
TABLE_NAME = "discrete_arithmetic_behavior_analysis"
...
def initialize_arithmetic_analysis_db() -> None:
    """
    Initialise the SQLite table for arithmetic behavior analysis.
    ...
    if result_df.empty:
        create_query = f"""
            CREATE TABLE {TABLE_NAME} (
                ...
                -- Analysis Fields (stored as JSON)
                obsessive_manual_calculation TEXT NOT NULL,
                fraction_conversion TEXT NOT NULL,
                scientific_notation TEXT NOT NULL,
                interpolation_or_estimation TEXT NOT NULL,
                arithmetic_hallucinations TEXT NOT NULL,
                constant_verification_loops TEXT NOT NULL,
                multiple_method_verification TEXT NOT NULL,
                excessive_precision TEXT NOT NULL,
...
def get_existing_arithmetic_analysis_identifiers() -> set[
...
def insert_arithmetic_analysis(
    query_uuid: str,
    naming_strategy: str,
    run: int,
    model_name: str,
    experiment_type: str,
    analysis_object: ArithmeticBehaviorAnalysis,
    ...
    """Insert a row into the discrete_arithmetic_behavior_analysis table."""
    # Extract fields from Pydantic model
    obsessive_manual_calculation = (
        analysis_object.obsessive_manual_calculation.model_dump_json()
    )
    fraction_conversion = analysis_object.fraction_conversion.model_dump_json()
    scientific_notation = analysis_object.scientific_notation.model_dump_json()
    interpolation_or_estimation = (
        analysis_object.interpolation_or_estimation.model_dump_json()
    )
    arithmetic_hallucinations = (
        analysis_object.arithmetic_hallucinations.model_dump_json()
    )
    constant_verification_loops = (
        analysis_object.constant_verification_loops.model_dump_json()
    )
    multiple_method_verification = (
        analysis_object.multiple_method_verification.model_dump_json()
    )
    excessive_precision = analysis_object.excessive_precision.model_dump_json()
    ...
    query = f"""
        INSERT INTO {TABLE_NAME} (
            ...
            obsessive_manual_calculation, fraction_conversion, scientific_notation,
            interpolation_or_estimation,
            arithmetic_hallucinations, constant_verification_loops,
            multiple_method_verification, excessive_precision,
            analysis_duration_ms,
            llm_call_full_prompt, llm_call_reasoning, llm_call_metadata,
            llm_call_model,
            llm_call_input_tokens, llm_call_output_tokens,
            llm_call_usage_metadata,
