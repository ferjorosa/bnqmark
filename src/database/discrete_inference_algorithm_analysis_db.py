"""Database utilities for the inference algorithm analysis table."""
...
# Constants
TABLE_NAME = "discrete_inference_algorithm_analysis"
...
def initialize_inference_analysis_db() -> None:
    """
    Initialise the SQLite table for inference algorithm analysis.
    ...
    if result_df.empty:
        create_query = f"""
            CREATE TABLE {TABLE_NAME} (
                ...
                barren_nodes TEXT NOT NULL,
                conditional_independence TEXT NOT NULL,
                algebraic_simplification TEXT NOT NULL,
...
def get_existing_inference_analysis_identifiers() -> set[
...
def insert_inference_analysis(
    query_uuid: str,
    naming_strategy: str,
    run: int,
    model_name: str,
    experiment_type: str,
    analysis_object: InferenceAlgorithmAnalysis,
    ...
    """Insert a row into the discrete_inference_algorithm_analysis table."""
    # Extract fields from Pydantic model
    classification = analysis_object.classification.model_dump_json()
    barren_nodes = analysis_object.barren_nodes.model_dump_json()
    conditional_independence = (
        analysis_object.conditional_independence.model_dump_json()
    )
    algebraic_simplification = (
        analysis_object.algebraic_simplification.model_dump_json()
    )
    ...
    query = f"""
        INSERT INTO {TABLE_NAME} (
            ...
            classification, barren_nodes, conditional_independence,
            algebraic_simplification,
