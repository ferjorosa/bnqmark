#!/usr/bin/env python3
"""
Script to generate a grid of accuracy heatmaps (3x3) in a single figure.
...
def create_heatmap_subplot(
    ...
    # Create warm colormap: white for 0, warm colors for higher values
    ...
    ax.set_yticklabels(
        [int(x) for x in accuracy_matrix.index],
        rotation=0,
...
def create_grid_figure(
    ...
    for i, model in enumerate(models):
        ax = axes[i]
        accuracy_matrix = all_matrices[model]
        create_heatmap_subplot(ax, accuracy_matrix, model)
    ...
    # Create warm colormap for colorbar
    colors = ["#FFFFFF", "#FFE5B4", "#FFA500", "#FF4500", "#8B0000"]
    n_bins = 100
    cmap = LinearSegmentedColormap.from_list("warm", colors, N=n_bins)
    ...
    if EXPERIMENT_TYPE == "raw_reasoning":
        title_text = f"Accuracy - Raw Reasoning (threshold = {ACCURACY_THRESHOLD})"
    elif EXPERIMENT_TYPE == "code_generation":
        title_text = f"Accuracy - Code Generation (threshold = {ACCURACY_THRESHOLD})"
    else:
        title_text = f"Accuracy - {EXPERIMENT_TYPE} (threshold = {ACCURACY_THRESHOLD})"
...
def save_plot(fig: plt.Figure, plots_dir: Path) -> Path:
    ...
    filename = (
        f"accuracy_heatmap_grid_{NAMING_STRATEGY}_{EXPERIMENT_TYPE}_"
...
def main():
    """Main function to generate accuracy heatmap grid."""
    print("=" * 70)
    print("Generating Accuracy Heatmap Grid (3x3)")
    print("=" * 70)
    print("Configuration:")
    ...
    # Calculate accuracy matrix for each model
    print("Calculating accuracy matrices for each model...")
    print("-" * 70)
    all_matrices = {}
    ...
    for model in models:
        ...
        accuracy_matrix = calculate_accuracy_matrix(model_df, queries_df, bns_df)
        all_matrices[model] = accuracy_matrix

    print(f"\n✓ Calculated {len(all_matrices)} accuracy matrices\n")
