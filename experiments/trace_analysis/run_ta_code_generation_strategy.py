#!/usr/bin/env python3
"""Code Strategy Trace Analysis Execution Script."""
...
from src.database.discrete_code_generation_strategy_analysis_db import (
    get_existing_code_generation_strategy_analysis_identifiers,
    initialize_code_generation_strategy_analysis_db,
)
...
def main():
    """Run the code generation strategy trace analysis."""
    ...
    config_path = (
        repo_root / "config" / "trace_analysis" / "code_generation_strategy.yaml"
    )
    prompts_path = (
        repo_root / "prompts" / "trace_analysis" / "code_generation_strategy.yaml"
    )
    ...
    logger.info("Initializing code generation strategy analysis database...")
    initialize_code_generation_strategy_analysis_db()
    ...
    logger.info(
        f"Starting Code Generation Strategy Analysis for models: {models_to_analyze}, "
        f"run: {run}"
    )

    # Get existing analyses to skip
    existing_analyses = get_existing_code_generation_strategy_analysis_identifiers()
    ...
    for model_to_analyze in models_to_analyze:
        ...
        run_trace_analysis_parallel(
            ...
            analysis_type=AnalysisType.CODE_STRATEGY,
    ...
    logger.info("Code Strategy Analysis completed!")
